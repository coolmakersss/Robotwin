...
DEFAULT_OUTPUT = "training_data_robomimic_transport_10d_action"
...
def arm_state_to_10d(position: np.ndarray, quaternion: np.ndarray, gripper: np.ndarray) -> np.ndarray:
    states = []
    for pos, quat, grip in zip(position, quaternion, gripper, strict=True):
        states.append(np.concatenate([pos, quaternion_to_rotation_6d(quat), np.array([grip])]))
    return np.asarray(states, dtype=np.float32)


def demo_state_to_20d(demo: h5py.Group, robot0_is_left: bool) -> np.ndarray:
    obs = demo["obs"]
    robot0 = arm_state_to_10d(
        obs["robot0_eef_pos"][()],
        obs["robot0_eef_quat"][()],
        gripper_qpos_to_open_amount(obs["robot0_gripper_qpos"][()]),
    )
    robot1 = arm_state_to_10d(
        obs["robot1_eef_pos"][()],
        obs["robot1_eef_quat"][()],
        gripper_qpos_to_open_amount(obs["robot1_gripper_qpos"][()]),
    )
    if robot0_is_left:
        return np.concatenate([robot0, robot1], axis=1).astype(np.float32)
    return np.concatenate([robot1, robot0], axis=1).astype(np.float32)
...
def write_episode(
    ...
    state = demo_state_to_20d(demo, robot0_is_left=robot0_is_left)
    if state.shape[0] < 2:
        return 0
    ...
    obs = demo["obs"]
    images = {
        "cam_high": obs["shouldercamera0_image"][:-1],
        "cam_left_wrist": obs["robot0_eye_in_hand_image"][:-1],
        "cam_right_wrist": obs["robot1_eye_in_hand_image"][:-1],
    }
    ...
    with h5py.File(hdf5_path, "w") as f:
        ...
        observations.create_dataset("left_arm_dim", data=np.full(qpos.shape[0], ARM_DIM, dtype=np.int32))
        observations.create_dataset("right_arm_dim", data=np.full(qpos.shape[0], ARM_DIM, dtype=np.int32))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Convert robomimic Transport image_v15.hdf5 to the pi0 10d_action intermediate format."
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Map robomimic robot0 to the left-arm slots and robot1 to the right-arm slots.",
    )
