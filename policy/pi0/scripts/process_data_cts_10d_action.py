...
def data_transform(path, episode_num, save_path, visualize_actions=True):
    ...
    for i in range(episode_num):
        ...
        for j in range(left_gripper_all.shape[0]):
            ...
            state_10d = cts_quat_to_cts_10d(cts_state).astype(np.float32)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process episodes into CTS 10d-per-arm action/state data.")
    parser.add_argument(
        "task_name",
        type=str,
    ...
    target_dir = args.save_dir or f"training_data_50_tasks_cts_10d_action/{task_name}-{setting}-{expert_data_num}"
    data_transform(
        load_dir,
        expert_data_num,
        target_dir,
