...
def data_transform(path, episode_num, save_path):
    ...
    for i in range(episode_num):
        ...
        for j in range(0, left_gripper_all.shape[0]):
            ...
            state_cts = cal_cts(end_pose_vector_all[j], Qa_last)  # cts
            Qa_last = state_cts[3:7]

            state = state.astype(np.float32)
            state_eef = state_eef.astype(np.float32)
            state_cts = state_cts.astype(np.float32)

            if j != left_gripper_all.shape[0] - 1:
                #qpos.append(state)
                qpos.append(state_cts)
...
if __name__ == "__main__":
    ...
    target_dir = f"training_data_cts/{task_name}-{setting}-{expert_data_num}"
